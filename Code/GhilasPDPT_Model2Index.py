...
def twoIndexModel(filename, seed=None, symm=False):
   ...
   """
    ...
    for (i,j) in A:
        if (j,i) in A:
            ...
            model.addConstr((z[i]-z[j]+x[i,j]*Mij[i,j]+(Mij[i,j]-t[i,j]+timeWindows[j][0]-timeWindows[i][1]-service_time[i])*x[j,i]<=Mij[i,j]-t[i,j]), name = "ct.time_flowLifted")
        else:
    ...
    """
    if HET or  MULTI_TRANS:
        model.addConstrs((z[i] -(Mij[i,j]+service_time[i])*(1-y[i,j,r])<= z[j] for i in TS for r in R for j in [a[1] for a in Ay.select(i,'*',0) if (a[0],a[1]) in TIy]), name = "ct.transferTimeWait")
        for tf in TFC:
            r = int(tf.split(".")[1])
   ...
   # model.addConstrs((timeWindows[i][0] <= a[i,r] for r in R for i in TS), name="ct.RtimeWindowEarliest")
    ...
    if HET or MULTI_TRANS:
        for ts in TSR:
            ...
            if (ts,ts.replace("s","f")) in A:
                model.addConstr((x[ts,ts.replace("s","f")]<=quicksum(y[ts,j,r] for j in [a[1] for a in Ay.select(ts,'*',r)])),name="symmbreak_Trans")
                if r>0:
    ...
    """
    for route in [[('o0', 'p15'), ('p15', 'p4'), ('p4', 'p18'), ('p18', 'd18'), ('d18', 'p21'), ('p21', 'd15'), ('d15', 'p14'), ('p14', 'd21'), ('d21', 'd4'), ('d4', 'd14'), ('d14', 'e0')],
[('o8', 'p2'), ('p2', 'p17'), ('p17', 'd2'), ('d2', 'p23'), ('p23', 'd23'), ('d23', 'd17'), ('d17', 'e8')],
[('o2', 'e2')],
[('o4', 'e4')],
[('o9', 'p1'), ('p1', 'p5'), ('p5', 'd5'), ('d5', 'p22'), ('p22', 'd1'), ('d1', 'd22'), ('d22', 'e9')],
[('o6', 'e6')],
[('o1', 'p13'), ('p13', 'd13'), ('d13', 'p11'), ('p11', 'd11'), ('d11', 'p6'), ('p6', 'd6'), ('d6', 'e1')],
[('o5', 'p3'), ('p3', 'p12'), ('p12', 'd3'), ('d3', 'd12'), ('d12', 'p9'), ('p9', 'd9'), ('d9', 'e5')],
[('o3', 'p0'), ('p0', 'p8'), ('p8', 'p24'), ('p24', 'p20'), ('p20', 'p19'), ('p19', 'd8'), ('d8', 'd24'), ('d24', 'd20'), ('d20', 'd19'), ('d19', 'd0'), ('d0', 'e3')],
[('o7', 'p7'), ('p7', 'd7'), ('d7', 'p10'), ('p10', 'p16'), ('p16', 'd16'), ('d16', 'd10'), ('d10', 'e7')]]:
        for e in route:
            model.addConstr(x[e]==1)#"""
    ...
    if model.Status == GRB.OPTIMAL:
        ...
        vehicle_empty= sum(model._x[i, j].x for o in TSC for (i,j) in model._A.select(o,"*"))
