...
filename = "./InstancesLyu23/PDPT/PDPT-R12-K2-T1/PDPT-R12-K2-T1-Q100-1.txt"
filename = "./InstancesGhilas/OriginalInstances/R20-K8-T2_GhilasOriginal.txt"
filename = "./InstancesGhilas/OriginalInstances/R5-K8-T2_GhilasOriginal_test.txt"
...
filename = "./InstancesGhilas/OriginalInstances/R30-K10-T2_GhilasOriginal.txt"
...
def twoIndexModelUB(filename,timeFlow=False, mode="two",symm=False, MULTI_TRANS=False):
    ...
    def create_pdptw_model(filename, cut=None, strCap=True,timeFlow=False, env=None, state=None):
             ...
             if VC<0:
                 model1.setObjective(0.5*quicksum(c[i,j]* x[i, j] for (i,j) in A2))
             else:
                 #if "Lim" in filename:
             ...
             if timeFlow==True:
                 ...
                 model1.addConstrs((z[i]+t[i,j]+service_time[j]-(Mij[i,j])*(1-x[i,j])<= z[j] for i in V for j in [a[1] for a in A2.select(i,'*')] if (j,i) not in A2), name = "ct.time_flowA")
                 #lifted
                 #model1.addConstrs((bz[i]-z[j]+Mij[i,j]*x[i,j]+(Mij[i,j]-t[i,j]-service_time[i]+min(-t[j,i],timeWindows[j][0]-timeWindows[i][1]))*x[j,i]<= Mij[i,j]-t[i,j]  for i in V for j in [a[1] for a in A2.select(i,'*')] if (j,i) in A2), name = "ct.time_flowLifted")
                 for (i,j) in A2:
                     if (j,i) in A2:
                         model1.addConstr((z[i]-z[j]+x[i,j]*Mij[i,j]+(Mij[i,j]-t[i,j]-service_time[j]+min(-t[j,i]-service_time[i],timeWindows[j][0]-timeWindows[i][1]-service_time[i]))*x[j,i]<=Mij[i,j]-t[i,j]-service_time[j]), name = "ct.time_flowLifted")

                         #model1.addConstr((z[i]-z[j]+x[i,j]*Mij[i,j]+(Mij[i,j]-t[i,j]-timeWindows[j][0]-timeWindows[i][1])*x[j,i]<=Mij[i,j]-t[i,j]-service_time[j]), name = "ct.time_flowLifted")
                 ...
                 model1.addConstrs((timeWindows[i][1] >= z[i]-service_time[i] for i in V), name="ct.TimeWindowLatestVD")
    ...
    def solve_compact(barrier=None, env=gp.Model(), timeFlow=False, mode="two", state=None, symm=False):
        ...
        if timeFlow==True:
            model_str += "_tf"
        model = Model(f'myTwoIndexModel_withUBPDPTW_StrCap_UB_{mode}Phase_superValidMTZ'+model_str,env=env )
        ...
        if VC<0:
            model.setObjective(0.5*quicksum(c[i,j] * x[i, j] for (i,j) in arcs))
        #else:
            #if "Lim" in filename:
        ...
        if timeFlow==True:
                ...
                if HET==False and symm==False:
                   model.addConstrs((z.sum(i,"*")+service_time[i] -Mij[i,j]*(1-ti[i,j])<= z.sum(j,"*") for (i,j) in TIy), name = "ct.transferTimeWait")
                    ...
                    if MULTI_TRANS==False:
                        model.addConstrs((z.sum(i,"*")+service_time[i] -M*(1-y[i,j,r])<= z.sum(j,"*") for r in R for (i,j) in TIy), name = "ct.transferTimeWait")
                    else:
                        model.addConstrs((z.sum(i,"*")+service_time[i] -Mij[i,j]*(1-y[i,j,r])<= z.sum(j,"*") for r in R for (i,j) in TIy), name = "ct.transferTimeWait")
            ...
            for (i,j) in A:
                if (j,i) in A:
                    model.addConstr((z[i]+service_time[j]+t[i,j]-Mij[i,j]*(1-x[i,j])<= z[j]), name = "ct.time_flow")
                    #model.addConstr((z[i]-z[j]+x[i,j]*Mij[i,j]+(Mij[i,j]-t[i,j]-timeWindows[j][0]-timeWindows[i][1])*x[j,i]<=Mij[i,j]-t[i,j]-service_time[j]), name = "ct.time_flowLifted")
                    #model.addConstr((z[i]-z[j]+x[i,j]*Mij[i,j]+(Mij[i,j]-t[i,j]-service_time[j]+min(-t[j,i]-service_time[i],timeWindows[j][0]-timeWindows[i][1]-service_time[i]))*x[j,i]<=Mij[i,j]-t[i,j]-service_time[j]), name = "ct.time_flowLifted")

                else:
                    model.addConstr((z[i]+t[i,j]+service_time[j]-Mij[i,j]*(1-x[i,j])<= z[j]), name = "ct.time_flow")
            #super valid inequality
            #model.addConstrs((z[i]+t[i,j]+service_time[j]-Mij[i,j]*(1-x[i,j])<= z[j] for (i,j) in A), name = "ct.time_flow")
            
            
            model.addConstrs((z[i]-(Mij[i,j]+service_time[i])*(1-y[i,j,r])<= z[j] for r in R for i in TS for j in [a[1] for a in Ay.select(i,'*',r) if (a[0],a[1]) in TIy]), name = "ct.transferTimeWait")
            for tf in TFC:
                r = int(tf.split(".")[1])
                model.addConstr((z[tf.replace("fc","sr")] <= z[tf]), name = "ct.transferTimeWait")
            """
            Time Windows
            """
        ...
        """
        for route in [[('o9', 'p4'), ('p4', 'p19'), ('p19', 'd4'), ('d4', 'd19'), ('d19', 'p14'), ('p14', 'd14'), ('d14', 'e9')],
[('o0', 'p8'), ('p8', 'd8'), ('d8', 'p5'), ('p5', 'd5'), ('d5', 'p10'), ('p10', 'p6'), ('p6', 'p0'), ('p0', 'd0'), ('d0', 'd10'), ('d10', 'd6'), ('d6', 'e0')],
[('o4', 'p12'), ('p12', 'p9'), ('p9', 'd12'), ('d12', 'd9'), ('d9', 'e4')],
[('o3', 'p21'), ('p21', 'p13'), ('p13', 'p23'), ('p23', 'd23'), ('d23', 'd13'), ('d13', 'p17'), ('p17', 'd17'), ('d17', 'd21'), ('d21', 'p24'), ('p24', 'd24'), ('d24', 'e3')],
[('o7', 'p28'), ('p28', 'p7'), ('p7', 'p25'), ('p25', 'p1'), ('p1', 'd1'), ('d1', 'd28'), ('d28', 'd7'), ('d7', 'd25'), ('d25', 'p26'), ('p26', 'd26'), ('d26', 'e7')],
[('o5', 'p3'), ('p3', 'p27'), ('p27', 'p15'), ('p15', 'd27'), ('d27', 'p16'), ('p16', 'd15'), ('d15', 'd3'), ('d3', 'd16'), ('d16', 'e5')],
[('o1', 'p2'), ('p2', 'p18'), ('p18', 'd18'), ('d18', 'd2'), ('d2', 'e1')],
[('o6', 'p29'), ('p29', 'p20'), ('p20', 'd29'), ('d29', 'd20'), ('d20', 'e6')],
[('o8', 'p22'), ('p22', 'd22'), ('d22', 'p11'), ('p11', 'd11'), ('d11', 'e8')],
[('o2', 'e2')]]:
            for e in route:
                model.addConstr(x[e]==1)
        #"""
        ...
        model._solver_state.infos_sol = infos[:-1]
        ...
        return infos[:-1]#.append(stats_RCCP)
