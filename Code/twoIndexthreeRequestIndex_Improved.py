...
filename = "./InstancesGhilas/HetBigPDPTWT_OrginalLoads/Ghilas_TC_R25-K10-T2.txt"
filename = "./InstancesGhilas/HetBigPDPTWT_OrginalLoads/Ghilas_TC_R30-K10-T2.txt"
#filename = "./InstancesATH/ath_25_3_3_1_L_converted.txt"
filename = "./InstancesGhilas/BigPDPTPWT/Ghilas__R35-K12-T1.txt"
...
def haversineCalc(lat1, lon1, lat2, lon2):
    speed = 20
    dist_m = haversine((lat1,lon1),(lat2,lon2), unit=Unit.METERS)
    dist_hm = np.ceil(dist_m / 100)
    km_ph = speed
    hm_ph = speed * 10
    hm_pm = hm_ph // 60
    return int(np.ceil(dist_hm / hm_pm))


# one unit of distance can be traveled in one time unit
def distancesMatrixAth(df):
    matrix = {}
    for location1 in df["node"]:
        for location2 in df["node"]:
            if location1 != location2:
                ...
                y1 = df.loc[df["node"]==location1, 'y'].values[0]
                y2 = df.loc[df["node"]==location2, 'y'].values[0]
                matrix[location1,location2] = haversineCalc(x1,y1, x2, y2)
    return matrix
...
def twoIndexModel(filename, seed=None, symm=False):
    ...
    def check_no_time_window_violation(i,j):
        ...
        if timeWindows[i][0]+t[i,j]+service_time[i]+service_time[j]<=timeWindows[j][1]:
            return True
        return False
    ...
    for i in TF:
        ...
        for j in TS:
            if j.split(".")[2]!=i.split(".")[2]:
                if HET== False and MULTI_TRANS==False:
    ...
    Mij = {(i,j):max(0,timeWindows[i][1]+t[i,j]-timeWindows[j][0]) for (i,j) in c}
    M = max(Mij.values())
    ...
    model = Model('myTwoIndexModel_TransferCuts_StrCap_bestPerforming_validMTZ')
    ...
    for i in V:
        for j in [a[1] for a in A.select(i,'*')]:
            if (j,i) in arcs:
                model.addConstr((bz[i]+c[i,j]+1e-3-(Mij[i,j]+1e-3)*(1-x[i,j])+(Mij[i,j]-1e-3-c[i,j]+min(-c[j,i]-1e-3,timeWindows[j][0]-timeWindows[i][0]))*x[j,i]<= z[j]), name = "ct.time_flowLifted")
            else:
                model.addConstr((bz[i]+c[i,j]+1e-3-(Mij[i,j]+1e-3)*(1-x[i,j])<= z[j]), name = "ct.time_flowA")
   ...
   """
    ...
    for (i,j) in A:
        if (j,i) in A:
                #model.addConstr((bz[i]+c[i,j]-Mij[i,j]*(1-x[i,j])<= z[j]), name = "ct.time_flow")

                model.addConstr((bz[i]-z[j]+x[i,j]*Mij[i,j]+(Mij[i,j]-t[i,j]+timeWindows[j][0]-timeWindows[i][1])*x[j,i]<=Mij[i,j]-t[i,j] ), name = "ct.time_flowLifted")
        else:
            model.addConstr((bz[i]+t[i,j]-Mij[i,j]*(1-x[i,j])<= z[j]), name = "ct.time_flow")
    """
    ...
    model.addConstrs((bz[i]-z[j]+Mij[i,j]*x[i,j]+(Mij[i,j]-t[i,j]+min(-t[j,i]-service_time[i]-service_time[j],timeWindows[j][0]-timeWindows[i][1]))*x[j,i]<= Mij[i,j]-t[i,j]  for i in V for j in [a[1] for a in A.select(i,'*')] if (j,i) in A), name = "ct.time_flowLifted")
    
    
    model.addConstrs((z[i]+service_time[i] <= bz[i] for i in V-(TS|TF)), name='ct.DepartureA')
    
    if HET:
        model.addConstrs((z[i]+service_time[i] <= bz[i] for i in TF|TS), name='ct.DepartureA')
        #model.addConstrs((z[i] <= bz[i] for i in TS), name='ct.DepartureA')
    else:
        model.addConstrs((z[i]+0.01+service_time[i] <= bz[i] for i in TF), name='ct.DepartureA')
        model.addConstrs((z[i]+0.01 <= bz[i] for i in TS), name='ct.DepartureA')
    ...
    """
    if HET or  MULTI_TRANS:
        model.addConstrs((z[i] -Mij[i,j]*(1-y[i,j,r])<= bz[j] for i in TS for r in R for j in [a[1] for a in Ay.select(i,'*',0) if (a[0],a[1]) in TIy]), name = "ct.transferTimeWait")
        for tf in TFC:
            r = int(tf.split(".")[1])
            model.addConstr((z[tf.replace("fc","sr")] <= bz[tf]), name = "ct.transferTimeWait")
    else:
   ...
   # model.addConstrs((timeWindows[i][0] <= a[i,r] for r in R for i in TS), name="ct.RtimeWindowEarliest")
    ...
    """
    for route in [[('o2', 'ts.2.0'), ('ts.2.0', 'tf.2.0'), ('tf.2.0', 'd4'), ('d4', 'd5'), ('d5', 'e2')],
    [('o0', 'p2'), ('p2', 'p4'), ('p4', 'p1'), ('p1', 'p5'), ('p5', 'ts.0.0'), ('ts.0.0', 'tf.0.0'), ('tf.0.0', 'd3'), ('d3', 'd2'), ('d2', 'e0')],
    [('o3', 'p3'), ('p3', 'p0'), ('p0', 'ts.3.0'), ('ts.3.0', 'tf.3.0'), ('tf.3.0', 'd1'), ('d1', 'd0'), ('d0', 'e3')]]:
        for e in route:
    ...
    if HET or MULTI_TRANS:
        ...
        #for ts in TSC:
            #r = int(ts.split(".")[1])
            #if r >0:
                #ts2 = ts.split(".")[0]+f".{r-1}."+ts.split(".")[-1]
                #model.addConstr(x[ts,ts.replace("s","f")]<=x[ts2,ts2.replace("s","f")])
    ...
    """
    for route in [[('o0', 'p2'), ('p2', 'p5'), ('p5', 'tsr.2.0'), ('tfr.2.0', 'd3'), ('d3', 'd2'), ('d2', 'e0')],
    [('o1', 'p4'), ('p4', 'd4'), ('d4', 'p3'), ('p3', 'tsr.3.0'), ('tfr.3.0', 'd0'), ('d0', 'e1')],
    [('o2', 'p0'), ('p0', 'tsr.0.0'), ('tfr.0.0', 'd5'), ('d5', 'e2')],
    [('o3', 'p1'), ('p1', 'd1'), ('d1', 'e3')]]:
        for e in route:
            model.addConstr(x[e]==1)"""
    # Data for callback
    ...
    if model.Status == GRB.OPTIMAL:
        ...
        infos = [filename, model.ModelName ,gurobi_status_dict.get(model.Status),model.ObjVal, model.MIPGap,model.ObjBound, model.Runtime,sol_transfers,sol_req_trans,vehicle_number,error_msg]  
        ...
        infos = [filename, model.ModelName ,gurobi_status_dict.get(model.Status),"inf","inf" ,model.ObjBound, model.Runtime,sol_transfers,sol_req_trans,vehicle_number,error_msg]
